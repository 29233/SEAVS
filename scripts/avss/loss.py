...
def Loss(pred_mask, gt_mask, gt_temporal_mask_flag, weight_dict):
    # pred_mask: l, bs * t, 1, h, w          # 改为直接用dice_loss和focal_loss的
    # gt_mask: bs x t, 1, h, w

    total_loss = 0
    mask_loss = F10_IoU_BCELoss(pred_mask, gt_mask, gt_temporal_mask_flag)

    total_loss = weight_dict['focal_loss'] * mask_loss

    loss_dict = {'focal_loss': mask_loss.item(), 'total_loss': total_loss.item()}
