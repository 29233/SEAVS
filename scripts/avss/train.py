...
def main():
    ...
    logger.info("Decoder params: %.2fM" % (sum(p.numel() for p in model.module.decoder.parameters()) / 1e6))
    ...
    for epoch in range(cfg.process.train_epochs):
        ...
        for n_iter, batch_data in enumerate(train_dataloader):
            ...
            # [bs*5, 24, 224, 224]
            output = model(audio, imgs, vid_temporal_mask_flag)
            loss, loss_dict = Loss(
                output, label, gt_temporal_mask_flag, cfg.loss.weight_dict)
            ...
            with torch.no_grad():
                ...
                if miou > max_miou:
                    model_save_path = os.path.join(
                        checkpoint_dir, '{}_miou_best_{}.pth'.format(args.session_name, cfg.discribe.info))
