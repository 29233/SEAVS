...
from loss import Loss, Context_align_Loss
...
def main():
    ...
    logger.info("VisualEncoder params: %.2fM" % (sum(p.numel() for p in model.visual_encoder.parameters()) / 1e6))

    input_v = torch.randn(5, 3, 224, 224).cuda()
    input_a = torch.randn(5, 1, 96, 64).cuda()
