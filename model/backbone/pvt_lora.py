...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, linear=False):
        ...
        rank = 32
        self.q = lora.Linear(dim, dim, bias=qkv_bias, r=rank)
        self.kv = lora.MergedLinear(dim, dim * 2, bias=qkv_bias, r=rank, enable_lora=[False, True])
...
class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 sr_ratios=[8, 4, 2, 1], num_stages=4, linear=False, init_weights_path=None):
        super().__init__()
        ...
        if init_weights_path is not None:
            self.initialize_weights(init_weights_path)

    def initialize_weights(self, path):
        pvt_model_dict = self.state_dict()
        pretrained_state_dicts = torch.load(path)
...
@register_model
def pvt_v2_lora(init_weights_path=False, **kwargs):
    model = PyramidVisionTransformerV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 6, 40, 3], sr_ratios=[8, 4, 2, 1],
        drop_rate=0.0, drop_path_rate=0.1, init_weights_path=init_weights_path,
        **kwargs)
