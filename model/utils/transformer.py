...
class AVSTransformerDecoderLayer(nn.Module):
    def __init__(self, dim=256, num_heads=8, ffn_dim=2048, dropout=0.1, num_levels=3, num_points=4, *args,
                 **kwargs) -> None:
        super().__init__()
...
class CAVSTransformerEncoderLayer(nn.Module):
    def __init__(self, dim=256, ffn_dim=2048, dropout=0.1, num_heads=8):
        super().__init__()

        # self attention
        self.self_attn = nn.modules.MultiheadAttention(dim, num_heads, dropout=dropout, batch_first=True)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(dim)
...
class CAVSTransformerEncoder(nn.Module):
    def __init__(self, num_layers, layer, *args, **kwargs) -> None:
        super().__init__()

        self.num_layers = num_layers
        self.layers = nn.ModuleList(
            [CAVSTransformerEncoderLayer(**layer) for i in range(num_layers)]
        )
...
class CAVSTransformerDecoderLayer(nn.Module):
    def __init__(self, dim=256, num_heads=8, ffn_dim=2048, dropout=0.1, num_levels=3, num_points=4, *args,
                 **kwargs) -> None:
        super().__init__()
...
class CAVSTransformerDecoder(nn.Module):
    def __init__(self, num_layers, layer, *args, **kwargs) -> None:
        super().__init__()

        self.num_layers = num_layers
        self.layers = nn.ModuleList(
            [CAVSTransformerDecoderLayer(**layer) for i in range(num_layers)]
        )
    ...
    def forward(self, query, src, padding_mask=None):
        ...
        outputs = []
        for layer in self.layers:
            out = layer(out, src, padding_mask)
            outputs.append(out)
        return outputs
class CAVSTransformer(nn.Module):
    def __init__(self, encoder, decoder, *args, **kwargs) -> None:
        super().__init__()

        self.encoder = CAVSTransformerEncoder(**encoder)
        self.decoder = CAVSTransformerDecoder(**decoder)
    ...
    def forward(self, query, src, pos=None, padding_mask=None):
        # TODO: change MSDeformableAttn to MultiheadAttention
        memory = self.encoder(src, pos)
        outputs = self.decoder(query, memory, padding_mask)
        return memory, outputs

def build_transformer(type, **kwargs):
    if type == 'AVSTransformer':
        return AVSTransformer(**kwargs)
    elif type == 'CAVSTransformer':
        return CAVSTransformer(**kwargs)
    else:
        raise ValueError
